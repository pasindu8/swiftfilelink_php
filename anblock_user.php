<?php
   include 'config.php';
	...
	if (hash_equals($expected_token, $url_token)) {
        ...
        {
            header("Location: alert.php?status=success&msg=User Unblock Successful!.&redirect=manage_users.php");
            exit();
        }
        else{
            header("Location: alert.php?status=error&msg=System Error:User Unblock Failed!&redirect=manage_users.php");
			exit();
        }
