...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px; max-width:600px; background-color:#0a0e1a; margin:2px; border-radius:16px; border:1px solid #3a2c0f; overflow:hidden; font-family: Arial, Helvetica, sans-serif; box-shadow:0 0 40px rgba(247,183,51,0.12);">
  ...
  <tr>
    <td style="height:5px; line-height:5px; font-size:0; background: linear-gradient(90deg, #00d4ff 0%, #0ffb56 30%, #f7b733 65%, #ff7a18 100%);">&nbsp;</td>
        ...
        $mail->send();
        setNotify('success', 'send email');
        header("Location: verification.php");
        exit();

    } catch (Exception $e) {
        setNotify('warning', 'System Error : Email could not be sent. Please try again.');
    }
}
...
if (isset($_GET['id']))
{
    ...
    $subtoken = mysqli_real_escape_string($conn, $_GET['id']);

    $tokencheck = "SELECT * FROM verification WHERE token='$subtoken'";
    ...
    if (mysqli_num_rows($result) > 0) 
    {
        ...
        sendOTP($email, $verificationcode, $name, $verid, $token);
    }
    else
    {
        setNotify('error', 'ERROR: Invalid security token reference.');
        header("Location: verification.php");
        exit();
    }
}
    ...
    {
    setNotify('error', 'ERROR: No token provided.');
    header("Location: verification.php");
    exit();
}
?>